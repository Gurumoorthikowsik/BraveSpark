...
class EmployeeReportController extends Controller{  
    ...
    function get_edit_report($id){
        ...
        $report = DB::table('user_info')->select('id','userid','employee_id','username','task','description','start_date','end_date','working_hours','report.status','created_at')->join('report', 'user_info.userid', '=', 'report.user_id')->where('userid',emp_user_id())->where('id',$id)->first();
    ...
    //     $check_permission = UserDocument::where('status','0')->where('id',$id);

    //         if($check_permission->count() != 0){

    //             $delete = UserDocument::where('id', $id)->delete();
