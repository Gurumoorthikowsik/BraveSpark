		        ...
		        $batchnumber = strtolower(trim($request['b_num']));
		        	...
		        	$check_email = User::select('email')->where('email',$staffemail)->where('status','!=',2);
				        ...
				        if($check_email->count() == 0){
						    ...
						    $emp_id_number = substr($get_emp_id->employee_id, 5); 

						    $employee_ids = (int)$emp_id_number + 1;  // Increment the numeric part

						    $employee_id = 'BSPRK' . str_pad($employee_ids, 4, '0', STR_PAD_LEFT);
    	...
    	if($_SERVER['REQUEST_METHOD'] === 'POST'){
    		...
    		if($check_user->count() != 0){
	    		...
	    		$updateData = [
	    			...
	    			'emergency_relationone'		=> null,
	    			'emergency_contactone'		=> null,
	    			'emergency_number'			=> null,
	    			'emergency_relationtwo'		=> null,
	    			'emergency_contactsecond'	=> null,
	    			'emergency_second'			=> null,
