...
{  
    ...
    public function index(Request $request ){
        ...
        $data['approved'] = Permission::select('hours')->where('status','=','Approved')->whereBetween('created_at', [$startDate, $endDate])->count();

        $data['pending'] = Permission::select('hours')->where('status','=','Pending')->whereBetween('created_at', [$startDate, $endDate])->count();
        

        $data['approved'] = Permission::select('hours')->where('status','=','Approved')->whereBetween('created_at', [$startDate, $endDate])->count();

        $data['pending_leave'] = Leave::select('user_id')->where('status','=','Pending')->whereBetween('created_at', [$startDate, $endDate])->count();

        $data['approved_leave'] = Leave::select('user_id')->where('status','=','Approved')->whereBetween('created_at', [$startDate, $endDate])->count();

        $data['pending_attendance_time_extended'] = Attendance_time_extended::select('user_id')->where('status','=','Pending')->whereBetween('created_at', [$startDate, $endDate])->count();

        $data['attendance_time_extended'] = Attendance_time_extended::select('user_id')->where('status','=','Approved')->whereBetween('created_at', [$startDate, $endDate])->count();

        $data['user_document'] = UserDocument::select('option')->where('option','0')->count();
